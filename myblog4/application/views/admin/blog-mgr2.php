...
<script>
    ...
    $(function () {
        ...
        function loadData() {
            ...
            $.get('admin/get_blogs?page=' + page++, function (res) {
                isEnd = res.isEnd;
                setTimeout(function () {
                    for (var i = 0; i < res.data.length; i++) {
                        var blog = res.data[i];
                        $('#tbody').append(_.template($('#tmpl').html())(blog));
                    }
        ...
        $('#btn-del').on('click',function(){
            var blog_ids= $('input:checked').not('#selectall');
            var anum=new Array();
            for(var i=0;i<blog_ids.length;i++){
                anum[i]=blog_ids.eq(i).val();
            }
            if (confirm('确定是否删除记录，不可恢复!?')) {
                $.get('admin/delete_all_blog', {
                    'anums': anum
                }, function (res) {
                    if (res == "success") {

                        blog_ids.parents('tr').remove();
