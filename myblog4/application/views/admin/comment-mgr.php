...
<script type="text/template" id="tmpl">
    <tr>
        <td><input  type="checkbox" value="<%= blog_id%>"/></td>
        <td><%= blog_id%></td>
        <td><%= comm_name%></td>
        ...
        <td>
            <div class="am-btn-toolbar">
                <div class="am-btn-group am-btn-group-xs">
                    <button data-id="<%= blog_id%>" class="am-btn am-btn-default am-btn-xs am-text-secondary btn-edit">
                        <span class="am-icon-pencil-square-o"></span> 回复
                    </button>
                    <button data-id="<%= blog_id%>"
    ...
    $(function () {
        $('#tbody').on('click', '.btn-delete', function () {
            ...
            if (confirm('确定是否删除记录，不可恢复!?')) {
                ...
                $.get('admin/delete_comment?comment_id=' + commentId, function (res) {
                    if (res == 'success') {
        ...
        $('#btn-add').on('click', function () {
            location.href = 'welcome/single';
        });

        $('#tbody').on('click', '.btn-edit', function(){
            var commentId = $(this).data('id');
            location.href = 'admin/replycomm_edit?comment_id=' + commentId;
        });
        ...
        function loadData() {
            ...
            $.get('admin/get_comment?page=' + page++, function (res) {
                isEnd = res.isEnd;
                console.log(res);
                setTimeout(function () {
                    for (var i = 0; i < res.data.length; i++) {
                        var comment = res.data[i];
                        $('#tbody').append(_.template($('#tmpl').html())(comment));
                    }
